...
The fastest way to get started is using Cortex TMS's official reusable workflow. This requires **zero configuration** and works out of the box.

#### Step 1: Create Your Workflow File

Create `.github/workflows/tms-validate.yml`:
...
- Install Cortex TMS automatically
- Validate your documentation
- Display results in GitHub Actions

<Aside type="tip" title="Zero Friction">
  The reusable workflow installs Cortex TMS globally, so you don't need to add it to your dependencies or build steps.
</Aside>

#### Step 2: Customize (Optional)

Add custom configuration with inputs:
...
#### Step 3: Pin to a Specific Version (Production)

For production use, pin to a specific version or tag:

```yaml
jobs:
  validate:
    uses: cortex-tms/cortex-tms/.github/workflows/validate-reusable.yml@v2.7.0
    # Or use a commit SHA for immutability:
    # uses: cortex-tms/cortex-tms/.github/workflows/validate-reusable.yml@abc123
```

<Aside type="note">
  Using `@main` gets the latest features but may introduce breaking changes. Pin to a version tag for stability.
</Aside>

---

### Option B: Custom Workflow (Advanced)

If you need more control or want to integrate Cortex TMS with your existing build process, create a custom workflow.
...
jobs:
  validate:
    ...
    steps:
      ...
      - run:
          name: Install cortex-tms
          command: npm install -g cortex-tms@3.1.0
      - run:
          name: Validate Cortex TMS
