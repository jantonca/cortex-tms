...
<Tabs>
  ...
  <TabItem label="Windows Permissions">
    ...
    **Option C: Add npm folder to exclusions**
    ...
    2. Manage settings → Add exclusion → Folder
    3. Add: `C:\Users\YourName\AppData\Roaming\npm`
    4. Run installation
  </TabItem>

  <TabItem label="Using sudo (Not Recommended)">
    **⚠️ Warning:** Using `sudo` with npm is a security risk. Only use as a last resort.
...
<Tabs>
  ...
  <TabItem label="Fix: Existing Files">
    ...
    ⚠️  Warning: 3 TMS file(s) already exist:
      - CLAUDE.md
      - NEXT-TASKS.md
      - .cortexrc
    ...
    **Option A: Use --force to overwrite**
    ...
    # ⚠️ This will overwrite existing TMS files!
    ```
...
- Symlinks don't work (`.cursorrules` → `CLAUDE.md`)
...
<Tabs>
  ...
  <TabItem label="Fix: Symlinks">
    ...
    **Option B: Copy instead of symlink**
    ...
    # ⚠️ Remember to update both files when editing
    ```
...
<Tabs>
  ...
  <TabItem label="Download Manually">
    ...
    **Step 3: Install from tarball**
    ...
    npm install -g ./cortex-tms-2.6.0.tgz
  ...
  <TabItem label="Disable SSL Verification (Risky)">
    **⚠️ Security Warning:** Only use as a last resort.
